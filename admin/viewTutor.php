            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        {
                            $tutor_id = mysqli_real_escape_string($conn, $_GET['Tutor_Id']);
                            $query = "SELECT * FROM tutor WHERE Tutor_Id='$tutor_id' ";
                            $query_run = mysqli_query($conn, $query);

                            if(mysqli_num_rows($query_run) > 0)
                            {
                                $tutor = mysqli_fetch_assoc($query_run);
                                ?>
                                <div class="mb-3">
                                    <label>Name:</label>
                                    <p class="form-control"><?= $tutor['T_Name']; ?></p>
                                </div>
                                <div class="mb-3">
                                    <label>Email:</label>
                                    <p class="form-control"><?= $tutor['T_Email']; ?></p>
                                </div>
                                <div class="mb-3">
                                    <label>Address:</label>
                                    <p class="form-control"><?= $tutor['T_Address']; ?></p>
                                </div>
                                <?php
                            }
                            else
